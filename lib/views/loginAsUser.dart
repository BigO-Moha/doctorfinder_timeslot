...
class _LoginAsUserState extends State<LoginAsUser> {
  ...
  loginInto(int type) async {
    if (EmailValidator.validate(phoneNumber) == false && type == 1) {
      ...
      dialog();
      log('data: "$SERVER_ADDRESS/api/login?email=$phoneNumber&password=$pass&login_type=$type&token=$token"');
      log('data: $SERVER_ADDRESS/api/login?email=$email&login_type=$type&token=$token&name=$name');
